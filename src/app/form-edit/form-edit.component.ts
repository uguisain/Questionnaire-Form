...
[x: string]: any;
  ...
  ngOnInit(): void {
    ...
    forkJoin({
        quiz: this.http.getApi<GetQuizRes>(`http://localhost:8080/quiz/get_quiz/${id}`),
        questions: this.http.getApi<GetQuestionRes>(`http://localhost:8080/quiz/get_question2?quizId=${id}`),
      }).subscribe({
        next: ({ quiz, questions }) => {
  ...
  addQuestion() {
    ...
    const nextQid = this.questions.length > 0
  ...
  saveUpdate() {
    ...
    if (!this.quiz.startDate.trim() || !this.quiz.endDate.trim() || this.quiz.startDate > this.quiz.endDate) {
